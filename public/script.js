document.addEventListener('DOMContentLoaded', () => {
  ...
  if (
    !textInput || !generateBtn || !loader || !resultArea ||
    !qrCodeImg || !downloadLink || !statusIndicator || !statusText ||
    !latencyValue || !uptimeValue || !versionValue || !lastCheckTime
  ) {
    console.error('Erro: Um ou mais elementos esperados não foram encontrados no DOM.');
    return;
  }
  ...
  const generateQRCode = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/generate?text=${encodeURIComponent(text)}`);
      if (!response.ok) {
  ...
  // Checa status da API ao carregar
  checkApiStatus();
